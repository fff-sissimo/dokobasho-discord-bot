...
const gateFor = ({ channelId, entry, allowlisted, worksheetStatus, categoryAllowlisted }) => {
  if (!allowlisted) return "skip:not_allowlisted";
  if (!entry) return "fail:registry_missing";
  if (categoryAllowlisted && entry.category_id) return "fail:category_allowlist_not_category";
...
const main = () => {
  ...
  const rows = channelIds.map((channelId) => {
    ...
    const categoryAllowlisted = allowedCategorySet.has(channelId);
    return {
      channel_id: channelId,
      type: entry ? entry.type : "unknown",
      status: statusText({ registryStatus, worksheetStatus }),
      allowlist: categoryAllowlisted ? "category" : categoryChild ? "category-child" : allowlisted ? "yes" : "no",
      gate: gateFor({ channelId, entry, allowlisted, worksheetStatus, categoryAllowlisted }),
    };
  });

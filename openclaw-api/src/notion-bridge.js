...
const READ_OPERATIONS = new Set(["search", "retrieve_page", "retrieve_block_children", "query_data_source"]);
const WRITE_OPERATIONS = new Set(["create_page", "append_blocks", "update_page_properties"]);
...
const createNotionBridge = ({ config = {}, fetchImpl = fetch, logger = console } = {}) => {
  ...
  const runRead = async (rawRequest = {}) => {
    ...
    try {
      if (operation === "search") {
        const query = normalizeText(rawRequest.query, 200);
        ...
        return {
          ...
          result: sanitizeResult({
            ...
            results: (result.results || []).slice(0, pageSize).map((item) => ({
              object: item.object,
              id: item.id,
              url: item.url,
  ...
  const runWrite = async (rawRequest = {}) => {
    ...
    try {
      ...
      if (operation === "update_page_properties") {
        const properties = rawRequest.properties && typeof rawRequest.properties === "object" && !Array.isArray(rawRequest.properties)
          ? rawRequest.properties
          : {};
        const serialized = JSON.stringify(properties);
        if (!serialized || serialized === "{}") return { ok: false, operation, reason: "notion_empty_properties" };
        if (/archived|in_trash/i.test(serialized)) {
          return { ok: false, operation, reason: "notion_destructive_write_denied" };
        }
        const result = await request(`/pages/${target.id}`, {
          method: "PATCH",
          body: { properties },
        });
        return { ok: true, operation, target_id_hash: hashIdentifier(target.id), page_id: result.id, url: result.url };

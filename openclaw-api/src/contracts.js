...
const buildDirectAgentPrompt = ({ payload, workspaceContext }) => [
  ...
  "Discord secret-backed workflow を使う場合は、通常テキストではなく必ず JSON だけを返し、`body` と `n8n_workflow_requests` を含めてください。",
  ...
  "Discord target は payload.guild_id / payload.channel.id / payload.channel.thread_id / payload.message.id から作ってください。current channel 読取・送信は target.guild_id と target.channel_id=payload.channel.id、current thread 読取・送信は target.thread_id=payload.channel.thread_id、current message から thread を作る場合だけ target.message_id=payload.message.id を入れてください。",
  "広域でない Discord 読取は、discord.fetch_recent_summary / discord.fetch_messages / discord.fetch_thread_messages に current channel または current thread の target を必ず入れてください。server-wide の summary/list だけ target なしを使えます。",
  ...
  "Notion / Discord workflow が不要な通常の web/workspace 作業は通常テキストで返して構いません。Notion または Discord secret-backed workflow 作業では必ず JSON で body と n8n_workflow_requests を返してください。",
  "",
  "# Runtime files",
  workspaceContext || "(no workspace context loaded)",
  "",
...
const buildDirectWorkflowResultPrompt = ({ payload, workspaceContext }) => [
  "あなたは Discord 上の `どこばしょのようせい` の OpenClaw direct handoff finalizer です。",
  "Discord へ直接投稿しないでください。Discord bot が返信するための最終報告だけを短く返してください。",
  "この turn では n8n workflow の追加依頼を作らないでください。`n8n_workflow_requests` は返さず、workflow_results を読んで最終文だけを返してください。",
  "workflow_results は openclaw-api が安全化した結果です。raw Discord 本文、未加工ログ、secret、token、個人情報を保存・出力しないでください。",
  "ユーザーの元依頼に対して、何を確認/作成/追記できたか、または不足情報を1つだけ短く返してください。",
  "返答に everyone/here、role mention、URL、添付、秘密値を含めないでください。",
  "",
  "# Runtime files",
  workspaceContext || "(no workspace context loaded)",
...
const parseAutonomyResponse = ({ eventType, stdout }) => {
  ...
  const [agentText] = extractAgentTexts(parsedStdout);

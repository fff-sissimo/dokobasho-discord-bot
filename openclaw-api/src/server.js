...
const buildSafeAutonomyCounts = (counts) => {
  ...
  return {
    checked_followups: normalizeAuditNumber(source.checked_followups) || 0,
    closed_followups: normalizeAuditNumber(source.closed_followups) || 0,
    dream_records: normalizeAuditNumber(source.dream_records) || 0,
...
const buildSafeAutonomyLogFields = ({ eventType, status, response, reason, errorCode }) => ({
  event_type: normalizeAuditCode(eventType, 40),
  status: normalizeAuditCode(status, 40),
...
const validateN8nWorkflowRequestForPayload = ({ payload, request }) => {
  ...
  if (workflowKey === "discord.server_read") {
    ...
    if (
      ["discord.fetch_recent_summary", "discord.list_channels", "discord.list_active_threads"].includes(request.operation) &&

...
- [ ] `getReminder(key, scope)`: 指定されたキーとスコープに一致するリマインダーを取得する関数。
- [ ] `listReminders(scope, query)`: 指定されたスコープのリマインダー一覧を取得する関数。
- [ ] `updateReminder(id, data)`: 指定IDの行データを更新する関数。スケジューラの排他制御で `status` を更新する際に重要。
- [ ] `deleteReminder(key, scope)`: 指定されたキーとスコープに一致するリマインダーを削除する関数。

...
- **引数:**
    - `time`: `string` (必須) - 通知時刻。自然言語（例: 「明日の15時」「10分後」「3日後」）および ISO 8601 形式 (`YYYY-MM-DDTHH:mm:ss`) を受け付ける。
...
- **挙動:**
    - `key` はシステムが8文字の英数字 (I/O/0/1 を除外) で自動生成し、登録完了メッセージと `/remind list` の一覧に表示する。
    - 既存のリマインダーで使用済みのキーは長さに関係なく有効とし、移行なしで `list`/`delete` に利用できる。
    - `time` のパースに失敗した場合は、具体的な入力例を提示してエラーを返す。
    - 登録成功時、登録内容のサマリーを返す。

### 1.2.2. `/remind get`（運用中止）

運用中止のため、現在は利用できない。必要な確認は `/remind list` を使用する。
...
- **挙動:**
    - 条件に合致するリマインダーを箇条書きで一覧表示する（項目: 自動生成された8文字のkey, 次回通知時刻, 繰り返し設定, contentの短縮版）。
...
- **表示形式:**
    - 成功時の応答は、簡潔なサマリーとする。
        - 例: `✅ リマインダー登録完了 — key: K7M2Z9H4 / 次回: 2026-01-11 15:00 (JST) / scope: channel`
    - エラー時の応答は、原因と対処法を具体的に示す。
        - 例: `❌ 時刻の指定が正しくありません。「明日の10時」や「2026-01-11 15:00」のように指定してください。`
    - `list` コマンドの出力は、キー、次回通知時刻、繰り返し、内容の先頭30文字を一覧表示する。
